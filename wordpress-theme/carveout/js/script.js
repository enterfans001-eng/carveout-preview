...
document.addEventListener('DOMContentLoaded', () => {
  const wpPageUrl = (path) => {
    const base = window.carveoutWpBaseUrl;
    ...
    const [filePath, query = ''] = pathAndQuery.split('?');
    const slug = filePath
      .replace(/^\.\//, '')
      .replace(/\.html$/, '')
      .replace(/^index$/, '')
    ...
    const pageUrl = slug ? `${normalizedBase}${slug}/` : normalizedBase;

    return `${pageUrl}${query ? `?${query}` : ''}${hash ? `#${hash}` : ''}`;
  };

  const themeAssetUrl = (path) => {
    const themeUrl = window.carveoutThemeUrl;

    if (!themeUrl || typeof path !== 'string') {
      return path;
    }
    ...
    const normalizedTheme = themeUrl.endsWith('/') ? themeUrl : `${themeUrl}/`;

    return `${normalizedTheme}${path.replace(/^\/+/, '')}`;
  };
  ...
  const initFloatingEntryCta = () => {
    ...
    cta.href = wpPageUrl('audition.html');
